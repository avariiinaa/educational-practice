...
open class Bor {
    ...
    protected var totalString: Int = 0
     ...
     */
    private fun go(node: Node, char: Char): Node {
    ...
    fun getIndexesOf(text: String): List<Pair<Int, Int>> {
        ...
        for (i in text.indices) {
            ...
            } // определяет все суффиксы, для корректной работы getSubPatterns
            // посмотреть есть ли слова которые является подстроками слова
...
fun main() {
    val bor = Bor()
    val text = readln()
    for (word in readln().substring(1).split('#')) {
        bor.insert(word)
    }
    if (LOG) print(bor.toString(tab = 0))
    if (LOG) println("\nAnswer:")
    println(bor.getIndexesOf(text).joinToString("\n") { "${it.second} ${it.first}" })
}
